...
import com.epam.movietheater.domain.DomainObject;
...
import com.epam.movietheater.service.AbstractDomainObjectService;
...
public class BookingState extends AbstractState {
    ...
    private void getBookedTickets() {
        ...
        printDelimiter();
        Set<TicketDTO> bookedTickets = bookingService.getPurchasedTicketsForEvent(event, airDate);
    ...
    private void bookTickets() {
        ...
        Set<TicketDTO> ticketsToBook = seats.stream().map(seat -> new TicketDTO(userForBooking, event, airDate, seat)).collect(Collectors.toSet());
        double price = bookingService.getTicketsPrice(event, airDate, user, seats, true);
    ...
    private void getTicketsPrice() {
        ...
        double price = bookingService.getTicketsPrice(event, airDate, user, seats, false);
        printDelimiter();
    ...
    private Set<Long> selectSeats(Event event, LocalDateTime airDate) {
        ...
        Set<TicketDTO> ticketDTOs = bookingService.getPurchasedTicketsForEvent(event, airDate);
        System.out.println(ticketDTOs);
        List<Long> bookedSeats = ticketDTOs.stream().map(t -> t.getSeat()).collect(Collectors.toList());
